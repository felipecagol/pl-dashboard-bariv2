...
def carregar_pnl_acumulado_oficial_completo(arquivo, cache_buster=None):
    ...
    for r in range(linha_metrica + 1, len(bruto)):
        ...
        for produto, cols in cols_produto.items():
            ...
            val_real = pd.to_numeric(bruto.iat[r, col_real], errors="coerce") if col_real and col_real < len(bruto.columns) else pd.NA
            val_orc = pd.to_numeric(bruto.iat[r, col_orc], errors="coerce") if col_orc and col_orc < len(bruto.columns) else pd.NA
...
def composicao_resultado_total_acumulado_produto(df_pnl_completo, periodo_atual, empresa_sel="Todos"):
    ...
    if empresa_sel == "Banco":
        ...
        if abs(valor_banco_digital) > 0.5:
            componentes.append(("Banco Digital", valor_banco_digital))
