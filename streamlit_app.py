...
def obter_periodos_pnl_mensal_anualizado(arquivo):
    ...
    # Cada mês é um "bloco" cujo topo traz a data e, 2 linhas abaixo, os produtos
    # (Consignado/Imobiliário/Total). Antes só o bloco de janeiro tinha o texto
    # "DATA BASE -->"; os demais trazem só a data. Por isso detectamos os blocos
    # pela própria data de cabeçalho, garantindo que todos os meses apareçam.
    for idx in bruto.index:
        for col in bruto.columns:
            data = converter_periodo(bruto.loc[idx, col])
            if data is None:
                continue
...
def carregar_pnl_mensal(arquivo):
    ...
    # Coluna onde ficam os nomes das linhas (ex.: "RECEITAS"). É compartilhada por
    # todos os blocos mensais (à esquerda da tabela).
    col_rotulo = None
    for c in bruto.columns:
        achou = False
        for r in bruto.index[:60]:
            if normalizar_texto(bruto.loc[r, c]) == "receitas":
                col_rotulo = c
                achou = True
                break
        if achou:
    ...
    if col_rotulo is None:
        col_rotulo = bruto.columns[0]
 
    blocos_processados = set()
 
    # Detecta cada bloco mensal pela data de cabeçalho (com produtos 2 linhas abaixo),
    # em vez de depender do texto "DATA BASE -->" — que só existe no bloco de janeiro.
    for idx in bruto.index:
        for col in bruto.columns:
            data_base = converter_periodo(bruto.loc[idx, col])
            if data_base is None:
                continue
 
            data_ts = pd.Timestamp(data_base)
            if data_ts.year < 2020 or data_ts.year > 2035:
                continue
            ...
            linha_produto = idx + 2
            linha_metrica = idx + 3
            if linha_metrica not in bruto.index:
                continue
 
            produtos_encontrados = {}
            for c_prod in range(col, min(col + 11, max(bruto.columns) + 1)):
            ...
            if chave_bloco in blocos_processados:
                continue
            blocos_processados.add(chave_bloco)
            ...
            for i, (produto, col_inicio) in enumerate(produtos_ordenados):
                col_fim = produtos_ordenados[i + 1][1] if i + 1 < len(produtos_ordenados) else min(col + 11, max(bruto.columns) + 1)
...
def variacao_pnl_acumulado_vs_2025(df_comp_2025, produto, linha, valor_ytd_atual):
    ...
    def buscar_valor_2025(norm_alvo):
        # Comissão (despesa): casamento exato, para não pegar "Receita Comissão CRI".
        if "comissao" in norm_alvo:
            base = df_comp_2025[
                (df_comp_2025["Ano"] == 2025)
                & (df_comp_2025["Produto"] == produto)
                & (df_comp_2025["Linha_Normalizada"] == "comissao")
            ]
            if not base.empty and pd.notna(base["Realizado"].iloc[0]):
    ...
    for ordem, linha_ref in enumerate(linhas_ordem):
        ...
        if linha_norm == normalizar_texto("DESPESAS TOTAIS"):
            ...
            def get_val(df_subset, l_norm, is_2026=False):
                # Comissão (despesa) tem o rótulo exato "comissao" e NÃO deve ser
                # confundida com "Receita Comissão CRI"; por isso o casamento exato.
                if "comissao" in l_norm:
                    r_exato = df_subset[df_subset["Linha_Normalizada"] == "comissao"]
                    if not r_exato.empty and pd.notna(r_exato["Realizado"].iloc[0]):
                        return r_exato["Realizado"].iloc[0]
                ...
                if "carteira" in l_norm:
                    r = df_subset[df_subset["Linha_Normalizada"].str.contains("carteira", na=False)]
                elif "pl medio" in l_norm:
                    r = df_subset[df_subset["Linha_Normalizada"].str.contains("pl medio", na=False)]
                elif "comissao" in l_norm:
                    r = df_subset[df_subset["Linha_Normalizada"].str.contains("comissao", na=False)]
                elif "amortizacao" in l_norm:
                    r = df_subset[df_subset["Linha_Normalizada"].str.contains("amortizacao", na=False)]
                elif "impostos diretos" in l_norm:
                    r = df_subset[df_subset["Linha_Normalizada"].str.contains("impostos diretos", na=False)]
                else:
                    return pd.NA
            ...
            if not df_2025_acumulado.empty and "Linha_Normalizada" in df_2025_acumulado.columns:
                ...
                if not b_acum.empty and pd.notna(b_acum["Realizado"].iloc[0]): 
                    ...
                    if "carteira" in linha_norm:
                        b_acum = df_2025_acumulado[df_2025_acumulado["Linha_Normalizada"].str.contains("carteira", na=False)]
                        ...
                        b_acum = df_2025_acumulado[df_2025_acumulado["Linha_Normalizada"].str.contains("impostos diretos", na=False)]
